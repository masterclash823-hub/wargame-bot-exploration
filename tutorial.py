...
CHAPTERS = [
    ...
    dict(key='economy', target='economy', pl=(
        ...
        'W prowincji można mieć po jednym budynku każdego typu, do poziomu 3. Domyślnie działa automat. W Gospodarka → Pracownicy możesz rezerwować ludzi dla budynków: 0 wyłącza obsadę, puste pole przywraca automat. Ręczne przydziały mogą zagrozić produkcji żywności. Normalne podatki wystarczą na start; wysokie zwiększają niepokoje i obniżają stabilność, niskie działają odwrotnie. Prognoza dotyczy miesiąca gry. Opcjonalna Polityka pracy pozwala wprowadzić lub znieść niewolnictwo: przed zatwierdzeniem pokazuje premie produkcji, koszty, niepokoje i skutki emancypacji.'), en=(
        ...
        'A province supports one building of each type, up to level 3. Automatic staffing is the default. In Economy → Workers, reserve workers for buildings: 0 leaves them unstaffed, an empty field restores automatic staffing. Manual assignments may put food production at risk. Normal taxes are enough to start; high taxes raise unrest and lower stability, while low taxes do the reverse. The forecast covers one game month. Optional Labor policy lets you introduce or abolish slavery; review output changes, costs, unrest and emancipation effects before confirming.')),
    ...
    dict(key='diplomacy', target='diplomacy', pl=(
        ...
        'Do wyboru: pokój, nieagresja, sojusz, dostęp wojskowy i gwarancja. Pokój może obejmować prowincje, złoto i reparacje. Zerwanie kosztuje 10 reputacji, a uzgodnione raty i dług pozostają. Gwarancja wymaga odpowiedzi na wezwanie przed kolejnym miesiącem gry; nie dołącza automatycznie do wojny. W aktywnym sojuszu przycisk Mariaż dynastyczny pozwala połączyć władcę, dorosłego syna lub córkę z postacią drugiej strony, za jej zgodą. Więź daje +0,25 stabilności/miesiąc; zerwanie takiego sojuszu kosztuje 20 reputacji i 5 stabilności. W pokoju można uzgodnić zwycięzcę wojny. Po zwycięstwie Wojsko → Jeńcy pokazuje dostępny limit oraz koszt zniewolenia; wymaga to polityki niewolnictwa.'), en=(
        '📜 Your seal matters',
        ...
        'Choose peace, non-aggression, alliance, military access or a guarantee. Peace can include provinces, gold and reparations. Breaking costs 10 reputation; agreed installments and debts remain payable. Guarantees require a response before the next game month and never join a war automatically. In an active alliance, Dynastic marriage links your ruler, adult son or daughter to the other dynasty with its consent. The bond grants +0.25 stability/month; breaking that alliance costs 20 reputation and 5 stability. Peace terms can name the war winner. After victory, Military → Captives shows the available limit and enslavement cost; this requires the slavery policy.')),
    ...
    dict(key='events', target='events', pl=(
        ...
        'Event trwa najwyżej 3 decyzje; własna odpowiedź także liczy się jako decyzja. Skutki są naliczane na końcu, a ich rzeczywiste wartości zapisywane w pamięci. Kolejne eventy mogą nawiązywać do wcześniejszych wyborów. Publiczne ogłoszenie nie ujawnia Twoich opcji ani odpowiedzi. Gdy DM nie dochodzą, użyj panelu. W Eksploracji opisujesz przygotowania i odpowiadasz wyłącznie własnymi słowami. Po maksymalnie 3 odpowiedziach wynik trafia na kanał rozpoczęcia z pingiem GM. Wznowienie: /exploration.'), en=(
        ...
        'An event takes at most 3 decisions; a custom answer counts too. Effects apply at the end, and actual changes are recorded in memory. Future events can refer to earlier choices. Public announcements do not reveal your options or answers. If DMs do not arrive, use the panel. In Exploration, describe preparations and reply only in your own words. After at most 3 replies, the result goes to the starting channel with a GM ping. Resume with /exploration.')),
